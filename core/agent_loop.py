import uuid, asyncio, logging, threading
...
async def run_agent_async(research_question, run_id=None, on_event=None, on_token=None, stream_tokens=False):
    ...
    async def _execute() -> ReportBundle:
        ...
        for idx, step in enumerate(other_steps):
            ...
            if step.kind in ("code", "exec"):
                ...
                if report.confidence_score < 0.4 and reroutes < settings.max_reroutes:
                    reroutes += 1
    ...
    try:
        return await asyncio.wait_for(_execute(), timeout=settings.run_timeout_seconds)
    except asyncio.TimeoutError:
        raise RuntimeError(
            f"Research run {run_id} exceeded timeout of {settings.run_timeout_seconds}s"
        )
